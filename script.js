...
/* ===========================
   INIT FLIPBOOK (robusto: evita width/height=0)
=========================== */
...
function createFlipbook(width, height, usePortrait){
  ...
  // doble update por barras del navegador móvil
...
function fit(){
  ...
  // Forzamos tamaño del contenedor para evitar 0
  root.style.width  = vw + 'px';
  root.style.height = vh + 'px';

  // Si por cualquier razón siguen en 0, reintenta en el siguiente frame
  ...
  // IMPORTANTE: width de PageFlip es el ANCHO DE UNA SOLA PÁGINA
  const pageW = isPortrait ? cw : Math.floor(cw / 2);
  const pageH = ch;

  // Si por cualquier razón salen 0, no intentes crear (reintenta)
  if (pageW < 2 || pageH < 2){
    requestAnimationFrame(fit);
    return;
  }
...
// Debounce + reintento tras cambios
let _tid;
